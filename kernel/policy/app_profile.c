...
int escape_with_root_profile(void)
{
    ...
    // Non-root profiles must retain their explicitly selected capabilities
    // across execve.  Root profiles intentionally keep the kernel's normal
    // inheritable/ambient handling so they cannot widen their capability set.
    if (profile->uid != 0) {
