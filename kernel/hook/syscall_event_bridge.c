...
static long __nocfi ksu_hook_execve_common(int orig_nr, const struct pt_regs *regs, bool execveat)
{
    ...
    if (current->pid != 1 && current_is_init) {
        ...
        ret = execveat ? ksu_handle_execveat_sucompat_tp_internal(filename_user, orig_nr, (struct pt_regs *)regs) :
                         ksu_handle_execve_sucompat_tp_internal(filename_user, orig_nr, (struct pt_regs *)regs);
